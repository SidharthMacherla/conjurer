...
### 2.2 Build customers
A customer is identified by a unique customer identifier(ID). A customer ID is an aplhanumeric with prefix "cust" followed by a numeric. This numeric ranges from 1 and extends to the number of customers provided as the argument within the function. For example, if there are 100 customers, then the customer ID will range from cust001 to cust100. This ensures that the customer ID is always of the same length.
...
### 2.2 Build products
The next step is building some products. A product is identified by a product ID. Similar to customer ID, a product ID is also an alphanumeric with prefix "sku" which signifies a stick keeping unit. This prefix is followed by a numeric ranging from 1 and extending all the way to the number of products provided as the argument within the function. For example, if there are 10 products, then the product ID will range from sku01 to sku10. This ensures that the product ID is always of the same length.
  Besides product ID, product price range must be specified. Let us build a group of products using the following code. For simplicity, let us assume that there are 10 products and the price range for them is from 5 dollars to 50 dollars. Products are built using the function buildProd. This function takes 3 arguments as given below.
  
  + numOfProd. This defines the number of product IDs to be generated.
  + minPrice. This is the minimum value if the price range.
...
Now that a group of customer IDs and Products are built, the next step is to build transactions. Transactions are built using the function genTrans. This function takes 5 arguments. The details of them are as follows.
  ...
  + **cylces**. This represents the cyclicality of data. It can take the following values
    * "y". If cycles is set to the value "y", it means that there is only one instance of high number of transactions during the entire year. This is a very common situation for some retail clients where the highest number of sales are during the holiday period in december.
    * "q". If cycles is set to the value "q", it means that there are 4 instances of high number of transactions. This is generally noticed in the financial services industry where the financial statements are revised every quarter and have an impact on the equity transactions in the secondary market.
    * "m". If cycles is set to the value "m", it means that there are 12 instances of high number of transactions for an year. This means that the number of transactions increase once every month and then subside for the rest of the month.
  ...
  + trend. This represents the slope of data distribution. It can take a value of 1 or -1. 
    + If trend is set to value 1, then the aggregated monthly transactions will exhibit an upward trend from January to December and vice versa if it is set to -1.
  + **outliers**. This signifies the presence of outliers. If set to value 1, then outliers are generated randomly. If set to value 0, then no outliers are generated. Presence of outliers is a very common occurence and hence setting the outliers to 1 is recommended. However, there are instances where outliers are not needed. For example, if the objective of data generation is solely for visualization purposes then outliers may not be needed.
...
Allocation of transactions is achieved with the help of buildPareto function. This function takes 3 arguments as detailed below.
...
Let us now allocate transactions to customers first by using following code.
```{r, eval=TRUE, echo=TRUE, results='markup'}
...
#### 2.4.3 Final data
The final step is to bring all this together into a final dataframe by using the following code.
```{r, eval=TRUE, echo=TRUE, results='markup'}
...
The column names of final dataframe can be interpretted as follows.
  + Each row is a transaction and the data frame has all the transaction for an year i.e 365 days.
...
The following is a view of sum of transactions by each day.
```{r, eval=TRUE, echo=TRUE, results='markup'}
aggregatedData <- aggregate(dfFinal$transactionID, by = list(dfFinal$dayNum), length)
plot(aggregatedData, type = "l", ann = FALSE)
